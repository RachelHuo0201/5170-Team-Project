...
Gmean_DT<- sqrt(Sensitivity * Specificity)
cat("F1-score_DT:", round(F1_DT, 4), "\n")
cat("G-mean_DT:", round(Gmean_DT, 4), "\n")
```
...
cm <- confusionMatrix(as.factor(pred_label), as.factor(test_y), positive = "1")
print(cm)
...
cat("F1-score_XGBoost:", round(F1, 4), "\n")
cat("G-mean_XGBoost:", round(Gmean, 4), "\n")
```
From the above result, we can see that the accuracy is around 0.8787, which means that this model correctly classified about 87.87% of the these wine samples as high or low quality. The F1 score for this model is around 0.5541 and G-mean of 0.6877, which mean sXGBoost is better at recognizing the balance between high and low. This result shows the XGBoost model has a strong overall predictive performance.
...
cm_RF <- confusionMatrix(predict_RF, all_test$label, positive = "high")
print(cm_RF)
...
Gmean_RF <- sqrt(recall_RF * specificity_RF)

cat("F1-score_RF:", round(F1_RF, 4),"\n")
cat("G-mean_RF:", round(Gmean_RF, 4),"\n")
```
From above result, we can see that the accuracy for random forest is 0.8695, which shows that this model correctly classified about 86.95% of the these wine samples as high or low quality.The F1 score of this model is around 0.4706 and G-means is 0.6096. Although the model correctly classifies most low-quality wines, it performs less effectively in identifying high-quality samples, reflecting a bias toward the majority class.


## Conclusion:
By comparing the above three tree-based models, we can see that XGBoost and Random Forest have higher accuracy than Decision Tree model. The best of these three model is XGBoost, which means this model has the strongest reliable predictive accuracy and we choose this model for this project. 

Furthermore, when comparing the results of training the models on separate datasets versus the combined dataset, we found that the combined dataset yielded significantly higher accuracy, suggesting that merging the red and white wine data provides a more comprehensive representation of wine characteristics and leads to better model performance.
